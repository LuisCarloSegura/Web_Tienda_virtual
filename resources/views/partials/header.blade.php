<header>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark custom-navbar-top py-3">
        <div class="container-fluid px-4">
            ...
            <form class="d-none d-sm-flex flex-grow-1 mx-4" action="{{ \Illuminate\Support\Facades\Route::has('productos.buscar') ? route('productos.buscar') : '#' }}" method="GET">
            ...
            <div class="d-flex align-items-center gap-3 ms-auto">
                @auth
                    <div class="dropdown d-none d-sm-block">
                        <a href="#" class="text-white text-decoration-none small fw-semibold dropdown-toggle d-flex align-items-center gap-2" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="{{ asset('images/avatar/avatar.png') }}" alt="Avatar {{ Auth::user()->nombre ?? Auth::user()->name }}" class="rounded-circle shadow-sm border border-2 border-white" style="width: 35px; height: 35px; object-fit: cover;">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 rounded-3 overflow-hidden" aria-labelledby="userDropdown" style="min-width: 220px;">
                            ...
                            @if(Auth::user()->esAdministrador())
                                <li>
                                    <a class="dropdown-item py-2 text-dark small fw-semibold d-flex align-items-center gap-2" href="{{ route('admin.productos.index') }}">
                ...
                @else
                    <a href="{{ \Illuminate\Support\Facades\Route::has('login') ? route('login') : '#' }}" class="text-white text-decoration-none small fw-semibold d-none d-sm-block">
                        ACCESO / REGISTRO
                    </a>
                @endauth

                {{-- Favoritos y carrito: solo para clientes o invitados, no para admin --}}
                @if(!Auth::check() || !Auth::user()->esAdministrador())
                    <a href="{{ \Illuminate\Support\Facades\Route::has('carrito') ? route('carrito') : '#' }}" class="text-white text-decoration-none d-flex align-items-center gap-2 position-relative">
        ...
        <div class="d-sm-none px-4 pt-3 w-100">
            <form action="{{ \Illuminate\Support\Facades\Route::has('productos.buscar') ? route('productos.buscar') : '#' }}" method="GET">
                <div class="input-group">
                    <span class="input-group-text bg-white border-0 rounded-start-pill ps-3">
                        <i class="bi bi-search text-secondary"></i>
                    </span>
                ...
                <ul class="navbar-nav align-items-lg-center gap-lg-4 py-2 py-lg-0">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu shadow">
                            ...
                            <li><a class="dropdown-item" href="{{ \Illuminate\Support\Facades\Route::has('categorias.show') ? route('categorias.show', 'audio') : '#' }}">Audio</a></li>
                            <li><a class="dropdown-item" href="{{ \Illuminate\Support\Facades\Route::has('categorias.show') ? route('categorias.show', 'tv') : '#' }}">TV y proyección</a></li>
                        </ul>
                    </li>
                    ...
                    <li class="nav-item">
                        <a class="nav-link fw-bold text-dark" href="{{ \Illuminate\Support\Facades\Route::has('nosotros') ? route('nosotros') : '#' }}">SOBRE NOSOTROS</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link fw-bold text-dark" href="{{ \Illuminate\Support\Facades\Route::has('pagos') ? route('pagos') : '#' }}">METODOS DE PAGO</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link fw-bold text-dark" href="{{ \Illuminate\Support\Facades\Route::has('contacto') ? route('contacto') : '#' }}">
